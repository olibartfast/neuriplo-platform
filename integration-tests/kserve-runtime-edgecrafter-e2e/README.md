...
`ovc` to generate OpenVINO IR when `model.xml` is missing, and exports or copies
`model.pte` for ExecuTorch when missing.
...
- ExecuTorch serves `model.pte` exported from the EdgeCrafter PyTorch checkpoint via
